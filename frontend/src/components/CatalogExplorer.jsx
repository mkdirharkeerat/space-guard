...
import { Search, Orbit, Globe, RefreshCw, Satellite, Radio, CheckCircle, ExternalLink } from 'lucide-react';
...
export default function CatalogExplorer({ onSelectObject }) {
  ...
  const fetchObjects = async () => {
    ...
    try {
      ...
      // Fallback sample orbital ephemeris
      const sampleSats = [
    ...
    <div className="flex flex-col gap-5 p-6 rounded-xl glass-panel border border-hud-border">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-hud-borderFaint">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-lg bg-hud-green/15 text-hud-green border border-hud-green/30 shadow-[0_0_10px_rgba(0,255,136,0.2)]">
            <Satellite className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-base font-bold text-white tracking-wide font-sans flex items-center gap-2">
              Live Tracked Objects & Ephemeris Catalog
            </h2>
        ...
        <div className="flex items-center gap-3">
          {dataAsOf && (
            <span className="text-[11px] text-slate-400 font-mono hidden md:inline-block">
              Propagated: {new Date(dataAsOf).toLocaleTimeString()} UTC
            </span>
          )}
          <button
            onClick={fetchObjects}
            disabled={loading}
            className="px-3 py-1.5 rounded-lg bg-deep border border-hud-border text-xs font-mono text-slate-200 hover:text-hud-green transition-all flex items-center gap-1.5"
          >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-void/90 border border-hud-border rounded-lg text-xs font-mono text-slate-100 placeholder-slate-500 focus:outline-none focus:border-hud-green focus:ring-1 focus:ring-hud-green transition-all"
        />
      </div>

      {/* Table of Propagated Satellites */}
      <div className="overflow-x-auto rounded-lg border border-hud-borderFaint">
        <table className="w-full text-left font-mono text-xs">
          <thead className="bg-deep/90 text-slate-400 border-b border-hud-borderFaint text-[11px] uppercase tracking-wider">
            <tr>
              <th className="py-3 px-4">NORAD ID</th>
              <th className="py-3 px-4">Satellite / Object</th>
              <th className="py-3 px-4">Position [X, Y, Z] (km)</th>
              <th className="py-3 px-4">Velocity [Vx, Vy, Vz] (km/s)</th>
              <th className="py-3 px-4">Altitude (LEO)</th>
              <th className="py-3 px-4 text-right">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-hud-borderFaint/40 bg-void/60">
            {filtered.map((sat) => {
              ...
              return (
                <tr key={sat.norad_id} className="hover:bg-hud-green/5 transition-colors">
                  <td className="py-3 px-4 font-bold text-hud-cyan">
                    #{sat.norad_id}
                  </td>
                  ...
                  <td className="py-3 px-4 text-hud-green font-semibold">
                    ~{altKm.toFixed(0)} km
                  </td>
                  <td className="py-3 px-4 text-right">
                    <button
                      ...
                      className="px-2.5 py-1 rounded bg-hud-green/10 text-hud-green border border-hud-green/30 hover:bg-hud-green hover:text-black font-semibold text-[11px] transition-all"
                    >
                      3D Track
                    </button>
                  </td>
