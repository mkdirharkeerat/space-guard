...
\Large \textbf{15 Questions Judges Will Ask --- And How to Answer Them}\\[0.5em]
...
This document has 15 questions that judges are very likely to ask you. Each question has:
\begin{itemize}
    \item A simple, clear answer you can say out loud.
    \item A tip on how to say it confidently.
\end{itemize}
Read through this the night before. You do not need to memorize exact words --- just understand the idea, and say it in your own natural way. If you do not know an answer, it is okay to say: ``That is a great point. In the current version we handle it this way... and in Phase 2 we plan to address that fully.''
\end{tcolorbox}
...
\begin{answerbox}
Every satellite's orbit is officially tracked and published as a ``Two-Line Element set'' --- basically two lines of coded text that describe the satellite's orbit. Space agencies like the US Space Force (USSPACECOM) generate these from radar tracking data and publish them freely.
...
\textbf{Step 1 (Height check):} Satellites can only collide if they fly at a similar height. We instantly discard all pairs where the two satellites orbit at very different heights. For example, a satellite at 400 km cannot hit one at 800 km --- they are in completely different orbital lanes. This eliminates over 90\% of pairs in milliseconds.

\textbf{Step 2 (Precise check):} For the small number of pairs that pass the height filter, we calculate exactly how close the two satellites will get over the next 72 hours, and at what time. If that closest distance is below a safe threshold, we compute the full collision probability.

This reduces 360 million comparisons down to a few hundred that actually need careful analysis.
\end{answerbox}

\begin{tipbox}
Great analogy: ``It is like sorting mail. First you sort by city (fast, eliminates most). Then you sort by street (more detail, fewer items). Then you find the exact house. We sort satellites by height first, then by exact path.''
\end{tipbox}
...
\begin{answerbox}
We know two things: (1) how close the satellites will get (their predicted closest distance), and (2) how uncertain our position measurements are (satellite tracking has small errors).

We model these uncertainties as a ``bell curve'' spread around the predicted position. If the two satellites are predicted to pass very close together AND our measurement uncertainty is large, the chance of an actual collision is higher. If they are predicted to pass far apart, the chance is very low.

The formula combines these factors into a single probability number. It is an established aerospace standard formula (called the Foster/Alfano method), and it computes in microseconds.

For the 2009 event, the two satellites were predicted to pass within 3 metres of each other, with measurement uncertainties of about 500 metres. That combination gives a 1-in-5,000 crash probability --- which our system correctly flags as Critical.
\end{answerbox}

\begin{tipbox}
If a judge asks you to write the formula on a whiteboard, you can write: $P_c \approx \frac{r^2}{2\sigma^2} e^{-d^2/(2\sigma^2)}$ where $r$ is the satellite size, $d$ is the closest distance, and $\sigma$ is the measurement uncertainty. But keep it simple and explain each variable.
\end{tipbox}
...
\begin{answerbox}
When you push a satellite slightly in the direction it is already flying (along its orbit), you change its orbital height very slightly. A higher orbit moves slightly slower. Over time, this means the satellite gradually falls behind --- it drifts along its path relative to where it would have been.
...
It is like steering a ship. A tiny rudder adjustment 10 km before a reef sends you safely around it. The same tiny adjustment 100 metres from the reef is not enough to avoid it.

So the exact same fuel, applied much earlier, gives 14 times more safety distance.
\end{answerbox}
...
\subsection*{Q10: Why did you not use AI / Deep Learning for everything?}

\begin{questionbox}
``Why did you not just train a neural network or AI model to predict collisions directly?''
\end{questionbox}

\begin{answerbox}
Because in safety-critical aerospace systems, you need to be able to explain and verify every decision. If a neural network tells an operator ``move your satellite'' but cannot explain exactly why, operators cannot trust it. If the AI is wrong, a satellite gets destroyed.

Our physics equations, on the other hand, are deterministic --- they always give the same answer for the same input, and every number can be checked and verified. The physics calculations are also faster than a neural network forward pass, so there is no speed advantage to AI here.

We do use a simple machine learning model (Random Forest), but only to quickly pre-sort which pairs are worth checking in detail. The actual crash probability and escape plan are always calculated using verified physics equations.
\end{answerbox}

\begin{tipbox}
This is a strong answer. Say: ``We use AI where it helps (fast pre-sorting), and we use physics where correctness is critical (the actual numbers). We do not use AI as a black box for safety decisions.'' Judges will appreciate this reasoning.
\end{tipbox}
...
\begin{answerbox}
\begin{itemize}
    \item \textbf{Step 1 (height filter)}: Under 5 milliseconds for 1,000 satellites.
    \item \textbf{Step 2 (precise closest distance)}: Run only on the small subset that passes the height filter. Takes a fraction of a second per pair.
...
\begin{tipbox}
You can demonstrate this live. In the demo, click ``Refresh'' on Step 1. The data appears in 1--2 seconds (including network time to fetch from the API). The physics calculation itself is sub-second. This is the real-time demo.
\end{tipbox}
...
\begin{enumerate}
    \item \textbf{Hit Refresh in Step 1}: The satellite position coordinates will update to the current UTC time. The positions change continuously because the satellites are actually moving.
...
\begin{tipbox}
Before the presentation, have a terminal window open with the backend logs visible. If a judge asks this question, offer to show the terminal. Live log output is very convincing proof that the system is actually calculating something.
\end{tipbox}
...
\begin{answerbox}
In practice, most close passes involve one active satellite and one piece of dead debris or a derelict satellite that cannot maneuver at all (like Cosmos 2251 in 2009). So usually only one satellite moves.

When both are active (like Starlink vs OneWeb), real-world operators follow international coordination protocols to decide which satellite moves and in which direction.
...
\begin{itemize}
    \item \textbf{Access to better data}: Full satellite position uncertainty data (covariance matrices from Space-Track CDMs), not just public TLE files.
...
\end{itemize}
The core physics, math, and algorithms are sound and validated on real data. The jump to operational deployment is a matter of engineering scale-up and institutional integration --- both of which are achievable.
\end{answerbox}

\begin{tipbox}
Say: ``This is a validated proof-of-concept. The hard part --- the physics and algorithms --- is working correctly on real historical data. The remaining steps are engineering scale-up, which is straightforward with resources. We have a clear Phase 2 and Phase 3 plan for this.''
\end{tipbox}

\vspace{1em}
\subsection*{Q15: What is Kessler Syndrome and how does Space-Guard help prevent it?}
...
\begin{tipbox}
This is a great question to answer because it puts the big picture of Space-Guard in perspective. Say it seriously: ``Kessler Syndrome is one of the most serious long-term threats to humanity's access to space. Every prevented collision matters. Space-Guard is one piece of the solution.''
\end{tipbox}
